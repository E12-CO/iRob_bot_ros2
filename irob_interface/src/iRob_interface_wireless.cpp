// iRob ROS2 wireless hardware interface code
...
class irob_rbc_if : public rclcpp::Node{
	...
	irob_rbc_if() : Node("iRob_Interface"){
		...
		declare_parameter("ip_address", "192.168.1.15");
		get_parameter("ip_address", ip_addr_);
		...
		if(i32iRobSocketFd < 0){
			RCLCPP_ERROR(
				this->get_logger(),
				"Error %d, can't create socket to connect to iRob",
				errno
			);
			std::raise(SIGTERM);
			return;
		...
		struct sockaddr_in irobServer;
		irobServer.sin_family 	= AF_INET;
		irobServer.sin_port 	= htons(IROB_TCP_PORT);
		irobServer.sin_addr.s_addr =
			inet_addr(ip_addr_.c_str());
		
		connect(
			i32iRobSocketFd, 
			(struct sockaddr *)&irobServer, 
			sizeof(irobServer)
			);
